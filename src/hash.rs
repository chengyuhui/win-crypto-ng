...
impl<Kind: AlgorithmKind> HashAlgorithm<Kind> {
    ...
    /// Open a hash algorithm provider in HMAC mode
    /// Do not use with [`MacAlgorithmId`]
    /// 
    /// [`MacAlgorithmId`]: ./enum.MacAlgorithmId.html
    pub fn open_hmac(id: Kind) -> Result<Self> {
        let handle = AlgoHandle::open_flags(id.to_str(), BCRYPT_ALG_HANDLE_HMAC_FLAG)?;
